...
The **Esway** app is part of the **eOffice Suite** — an open-source AI-powered office productivity stack with built-in eBot LLM integration.
...
```bash
pnpm --filter esway test     # if vitest is configured
pnpm --filter esway build    # production bundle (dist/)
```
