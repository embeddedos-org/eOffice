...
export class EmailService {
  ...
  async sendMessage(to: string, subject: string, body: string, html?: string, attachments?: Array<{ filename: string; content: Buffer | string; contentType?: string }>): Promise<{ messageId: string }> {
  ...
  async moveToTrash(folder: string, uid: number): Promise<void> {
    ...
    try {
      ...
      // Fallback: try standard Trash folder
      try {
