...
The **Email** app is part of the **eOffice Suite** — an open-source AI-powered office productivity stack with built-in eBot LLM integration.
...
```bash
pnpm --filter email test     # if vitest is configured
pnpm --filter email build    # production bundle (dist/)
```
