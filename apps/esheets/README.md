...
The **Esheets** app is part of the **eOffice Suite** — an open-source AI-powered office productivity stack with built-in eBot LLM integration.
...
```bash
pnpm --filter esheets test     # if vitest is configured
pnpm --filter esheets build    # production bundle (dist/)
```
