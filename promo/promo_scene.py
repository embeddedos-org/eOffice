"""eOffice — production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eOffice", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Complete Productivity Suite", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "TypeScript, Electron, React".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "Document Editor", "Rich text editing with real-time collaboration, track changes, and export to PDF", DUR["f1"]),
            ("02", "Spreadsheet Engine", "Formula engine with 400 plus functions, pivot tables, and chart visualization", DUR["f2"]),
            ("03", "Presentation Builder", "Slide editor with templates, animations, and presenter mode", DUR["f3"]),
        ]
        ...
        components = ["Editor Core", "Renderer", "Collaboration", "Storage", "Export"]
        boxes = VGroup()
