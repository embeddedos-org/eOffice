...
The **Enotes** app is part of the **eOffice Suite** — an open-source AI-powered office productivity stack with built-in eBot LLM integration.
...
```bash
pnpm --filter enotes test     # if vitest is configured
pnpm --filter enotes build    # production bundle (dist/)
```
