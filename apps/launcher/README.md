...
The **Launcher** app is part of the **eOffice Suite** — an open-source AI-powered office productivity stack with built-in eBot LLM integration.
...
```bash
pnpm --filter launcher test     # if vitest is configured
pnpm --filter launcher build    # production bundle (dist/)
```
